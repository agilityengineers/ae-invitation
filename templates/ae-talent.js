(function(){
  if(window.__aeTalAdminInit)return; window.__aeTalAdminInit=1;
  var KEY='aeTalentConfig_v1';
  ...
  window.aeTalentAdmin={get:function(){return state;},apply:apply};
})();

/* ===================== LEAD QUALIFIER ENGINE (v3) ===================== */
(function(){
  if(window.__aeTalQuizInit)return; window.__aeTalQuizInit=1;
  var QKEY='aeTalentQuizConfig_v1', SKEY='aeTalentQuizSettings_v1', LKEY='aeTalentLeads_v1';
  var TEAL='#0F88A2',NAVY='#08527F',NAVY7='#063A5A',GREEN='#2E8B57',BG='#F2F5F7',INK='#0B2A38',MUT='#5A6B73',LINE='#E2E8EB';
  var DEFAULTQUIZ=[
    {id:'role',q:'Which role best describes you?',options:[
    ...
    {id:'experience',q:'How many years of professional experience do you have?',options:[
    ...
    {id:'focus',q:'Where do you do your strongest work?',options:[
    ...
    {id:'availability',q:'What is your current availability?',options:[
    ...
    {id:'model',q:'How do you prefer to work?',options:[
    ...
    {id:'agile',q:'How would you describe your experience on agile delivery teams?',options:[
  ...
  var DEFSET={schedProvider:'directory',schedMode:'link',schedUrl:'https://www.agility-engineers.com/',schedEmbed:'',killOn:false,killTiming:'end',showScore:false,resourceOn:false};
  ...
  function renderLead(N){
    ...
    body.appendChild(el('h2','font:800 clamp(20px,2.4vw,26px)/1.2 \'Archivo\',sans-serif;color:'+NAVY+';margin:0 0 6px','Where should we send your directory profile?'));
    body.appendChild(el('p','font:500 14.5px/1.5 \'IBM Plex Sans\',sans-serif;color:'+MUT+';margin:0 0 20px','We\u2019ll show your directory fit, then take you straight to complete your Agility Engineers directory profile \u2014 no spam, no list.'));
    ...
    var go=el('button','width:100%;border:none;background:'+GREEN+';color:#fff;font:700 16px \'Archivo\',sans-serif;padding:16px;border-radius:11px;cursor:pointer;box-shadow:0 12px 26px rgba(46,139,87,.3)','See my directory fit & sign up \u2192');
  ...
  function openScheduler(){
    ...
    alert('Directory signup link not configured yet. An admin can add the directory URL (or an embed) in the Page admin panel.');
  }
  function renderEmbedPage(s){
    while(card.firstChild)card.removeChild(card.firstChild);
    card.style.maxWidth='860px';
    card.appendChild(topBar('Complete my directory profile'));
  ...
  function renderResult(N){
    ...
    card.appendChild(topBar('Your Directory Fit'));
    ...
    var of=el('div','font:600 15px \'IBM Plex Sans\',sans-serif;color:'+MUT,'/ 100\nDirectory Fit Score'.split('\n')[0]);
    var scoreWrap=el('div',null);scoreWrap.appendChild(num);
    var lab=el('div','font:600 13px \'IBM Plex Sans\',sans-serif;color:'+MUT+';margin-top:2px','out of 100 \u00b7 Directory Fit Score');
    ...
    var msg=r.tier==='elite'
      ?'Your background is a strong match for the Agility Engineers directory. Finish your profile to get listed and visible to teams looking for people like you.'
    ...
    var go=el('button','width:100%;border:none;background:'+GREEN+';color:#fff;font:700 16px \'Archivo\',sans-serif;padding:16px;border-radius:11px;cursor:pointer;box-shadow:0 12px 26px rgba(46,139,87,.3)','Complete my directory profile \u2192');
    go.addEventListener('click',function(){openScheduler();});
    body.appendChild(go);
    body.appendChild(el('p','font:500 12.5px/1.5 \'IBM Plex Sans\',sans-serif;color:'+MUT+';text-align:center;margin:12px 0 0','A complete profile keeps your directory listing active and visible to hiring teams.'));
    card.appendChild(body);
  }
  function renderBooking(r,s){
    card.appendChild(topBar(r.tier==='moderate'?'One step before we build':'You\u2019re a strong fit'));
    var body=el('div','padding:28px 26px');
    body.appendChild(el('h2','font:800 24px/1.2 \'Archivo\',sans-serif;color:'+NAVY+';margin:0 0 10px',r.tier==='moderate'?'Let\u2019s finish your directory profile':'Let\u2019s join the Agility Engineers directory'));
    var msg=r.tier==='moderate'
      ...
      :'Your background is a strong match for our directory. Complete your profile to get listed and discoverable by hiring teams.';
  ...
  function injectAdmin(){
    ...
    var sec=el('div',null);
    sec.appendChild(chip('Directory intake \u2192 booking'));
    sec.appendChild(el('div','font:500 12px/1.4 \'IBM Plex Sans\',sans-serif;color:#8a9aa2;margin-bottom:10px','Every CTA opens the directory intake, then routes the applicant. Configure the flow here.'));
